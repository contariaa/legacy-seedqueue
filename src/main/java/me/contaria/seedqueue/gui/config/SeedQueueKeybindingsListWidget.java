...
public class SeedQueueKeybindingsListWidget extends EntryListWidget {
    ...
    public abstract static class Entry implements class_1803 {
        @Override
    ...
    public class CategoryEntry extends Entry {
        ...
        public CategoryEntry(String text) {
            this.text = text;
            this.textWidth = MinecraftClient.getInstance().textRenderer.getStringWidth(this.text);
        }
        ...
        public void method_6700(int index, int x, int y, int entryWidth, int entryHeight, Tessellator tessellator, int mouseX, int mouseY, boolean hovered) {
            MinecraftClient.getInstance().textRenderer.draw(this.text, (SeedQueueKeybindingsListWidget.this.parent.width - this.textWidth) / 2, y + entryHeight - MinecraftClient.getInstance().textRenderer.fontHeight - 1, 0xFFFFFF);
        }
    }
    ...
    public abstract class KeyEntry extends Entry {
        ...
        public boolean method_6699(int index, int mouseX, int mouseY, int button, int x, int y) {
            for (ButtonWidget widget : this.children()) {
                if (widget.isMouseOver(MinecraftClient.getInstance(), mouseX, mouseY)) {
    ...
    public class PrimaryKeyEntry extends KeyEntry {
        ...
        private List<String> createAdditionalKeysText(String translationKey, List<Integer> keys) {
            ...
            if (keys.isEmpty()) {
                text2.append(I18n.translate("seedqueue.menu.keys.none"));
            } else {
            ...
            if (MinecraftClient.getInstance().textRenderer.getStringWidth(combined) < maxWidth - 10) {
        ...
        @Override
        public void method_6700(int index, int x, int y, int entryWidth, int entryHeight, Tessellator tessellator, int mouseX, int mouseY, boolean hovered) {
            super.method_6700(index, x, y, entryWidth, entryHeight, tessellator, mouseX, mouseY, hovered);
        ...
        @Override
        public boolean method_6699(int index, int mouseX, int mouseY, int button, int x, int y) {
            if (super.method_6699(index, mouseX, mouseY, button, x, y)) {
                return true;
            }
